...
//                getKVStore().put(key, value);
    ...
    /*public List<GenreTO> getGenres() {
        ...
        while (keyValueIter.hasNext()) {
            ...
            System.out.println(genreTOValue);
            genreTO = new GenreTO(genreTOValue);
            //Add TO to the list
            genreList.add(genreTO);
            TableInfo tableInfo =new TableInfo("info","cid","GN");
            TableInfo tableInfo1 =new TableInfo("info", "id", String.valueOf(genreTO.getId()));
            TableInfo tableInfo2 =new TableInfo("info","name",genreTO.getName());
            //System.out.println(genreTO.getId() + " " + genreTO.getGenreJsonTxt());
            list.add(tableInfo);
            list.add(tableInfo1);
            list.add(tableInfo2);
            HbaseUtil.put("genre", String.valueOf(i), list);
            i++;
        } //EOF while
        return genreList;
    }*/
    public List<GenreTO> getGenres(){
    	...
    	String genreTOValue = null;  
    	List<GenreTO> genreList = new ArrayList<GenreTO>();
    	List<Result> results = HbaseUtil.getAll("genre");
        Iterator<Result> iter = results.iterator();
        while(iter.hasNext())
        {
        	Result result = iter.next();
        	genreTOValue = HbaseUtil.resultToJsonString(result);
